...
const envKeyObj = await subtle.importKey("raw",
Buffer.from(ENV_KEY_KEY, 'utf8'), {name:'AES-CBC'}, false,
['encrypt','decrypt']);
...
async function generateJWKS(){
    ...
    */
    ...
    for(const k of allKeys){
        let keyBuf:Buffer = k.dataValues.key;
...
app.all('/register', async (req, res, err)=>{
    ...
    let reg:UserRegistration = req.body;
